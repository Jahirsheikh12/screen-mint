...
export function ExportDialog({
	...
	return (
		<>
			<div
				className={`fixed inset-0 bg-black/80 backdrop-blur-md z-50 transition-opacity duration-200 ${isClosing ? "opacity-0" : "animate-in fade-in duration-200"}`}
				onClick={isExporting ? undefined : onClose}
			/>
			<div
				className={`fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-[60] bg-[#0A0F0D] rounded-2xl shadow-2xl border border-white/10 p-8 w-[90vw] max-w-md transition-all duration-200 ${isClosing ? "opacity-0 scale-95" : "animate-in zoom-in-95 fade-in duration-200"}`}
			>
				<div className="flex items-center justify-between mb-6">
					<div className="flex items-center gap-4">
						{showSuccess ? (
							<>
								<div className="w-12 h-12 rounded-full bg-[#10B981]/20 flex items-center justify-center ring-1 ring-[#10B981]/50">
				...
				{error && (
					<div className="mb-6 animate-in slide-in-from-top-2">
						...
						{!isExporting && canRetrySave && onRetrySave && (
							<Button
								onClick={onRetrySave}
								className="w-full mt-3 bg-[#10B981] text-white hover:bg-[#1D4ED8]"
							>
								Save Again
							</Button>
						)}
						...
						<div className="space-y-2">
							...
							<div className="h-2 bg-white/5 rounded-full overflow-hidden border border-white/5">
								{isCompiling || isFinalizing ? (
									...
									renderProgress !== undefined && renderProgress > 0 ? (
										<div
											className="h-full bg-[#10B981] shadow-[0_0_10px_rgba(16,185,129,0.3)] transition-all duration-300 ease-out"
											style={{ width: `${renderProgress}%` }}
										/>
									) : (
										<div className="h-full w-full relative overflow-hidden">
											<div
												className="absolute h-full w-1/3 bg-[#10B981] shadow-[0_0_10px_rgba(16,185,129,0.3)]"
												style={{
