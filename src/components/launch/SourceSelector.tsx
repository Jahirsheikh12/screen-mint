    ...
    <div className={`min-h-screen flex flex-col items-center justify-center ${styles.glassContainer}`}>
      <div className="flex-1 flex flex-col w-full max-w-xl" style={{ padding: 0 }}>
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'screens' | 'windows')}>
          <TabsList className="grid grid-cols-2 mb-3 bg-zinc-900/40 rounded-full">
            <TabsTrigger value="screens" className="data-[state=active]:bg-[#10B981] data-[state=active]:text-white text-zinc-200 rounded-full text-xs py-1">
              {t('sourceSelector.screens')} ({screenSources.length})
            </TabsTrigger>
            <TabsTrigger value="windows" className="data-[state=active]:bg-[#10B981] data-[state=active]:text-white text-zinc-200 rounded-full text-xs py-1">
                      ...
                      <div className="relative mb-1">
                        ...
                        {selectedSource?.id === source.id && (
                          <div className="absolute -top-1 -right-1">
                            <div className="w-4 h-4 bg-[#10B981] rounded-full flex items-center justify-center shadow-md">
      ...
      <div className="border-t border-zinc-800 p-2 w-full max-w-xl">
        <div className="flex justify-center gap-2">
          ...
          <Button onClick={handleShare} disabled={!selectedSource} className="px-4 py-1 text-xs bg-[#10B981] text-white hover:bg-[#10B981]/80 disabled:opacity-50 disabled:bg-zinc-700">{t('sourceSelector.share')}</Button>
        </div>
      </div>
    </div>
  );
