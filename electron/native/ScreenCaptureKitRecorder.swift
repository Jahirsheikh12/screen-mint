...
final class ScreenCaptureRecorder: NSObject, SCStreamOutput, SCStreamDelegate {
	private let queue = DispatchQueue(label: "screenmint.screencapturekit.video")
	...
	func startCapture(configJSON: String) async throws {
		guard !isRecording else {
			throw NSError(domain: "ScreenMintCapture", code: 1, userInfo: [NSLocalizedDescriptionKey: "Recording is already in progress"])
		}

		guard let data = configJSON.data(using: .utf8) else {
			throw NSError(domain: "ScreenMintCapture", code: 2, userInfo: [NSLocalizedDescriptionKey: "Invalid JSON input"])
		}
		...
		if let windowId = config.windowId {
			...
			guard let window = availableContent.windows.first(where: { $0.windowID == windowId }) else {
				throw NSError(domain: "ScreenMintCapture", code: 3, userInfo: [NSLocalizedDescriptionKey: "Window not found"])
			}
			...
			guard let display = availableContent.displays.first(where: { $0.displayID == displayId }) else {
				throw NSError(domain: "ScreenMintCapture", code: 4, userInfo: [NSLocalizedDescriptionKey: "Display not found"])
			}
		...
		guard let assistant = AVOutputSettingsAssistant(preset: .preset3840x2160) else {
			throw NSError(domain: "ScreenMintCapture", code: 5, userInfo: [NSLocalizedDescriptionKey: "Unable to create output settings assistant"])
		}
		...
		guard var outputSettings = assistant.videoSettings else {
			throw NSError(domain: "ScreenMintCapture", code: 6, userInfo: [NSLocalizedDescriptionKey: "Output settings unavailable"])
		}
		...
		guard let assetWriter = assetWriter, assetWriter.canAdd(videoInput) else {
			throw NSError(domain: "ScreenMintCapture", code: 7, userInfo: [NSLocalizedDescriptionKey: "Unable to add video writer input"])
		}
		...
		if primaryAudioSource != nil {
			...
			guard assetWriter.canAdd(audioInput) else {
				throw NSError(domain: "ScreenMintCapture", code: 11, userInfo: [NSLocalizedDescriptionKey: "Unable to add audio writer input"])
			}
		...
		if writesMicrophoneToSeparateTrack {
			guard let microphoneOutputPath = config.microphoneOutputPath, !microphoneOutputPath.isEmpty else {
				throw NSError(domain: "ScreenMintCapture", code: 12, userInfo: [NSLocalizedDescriptionKey: "Missing microphone output path for dual-audio capture"])
			}
			...
			guard microphoneWriter.canAdd(microphoneInput) else {
				throw NSError(domain: "ScreenMintCapture", code: 13, userInfo: [NSLocalizedDescriptionKey: "Unable to add microphone writer input"])
			}
			...
			guard microphoneWriter.startWriting() else {
				throw NSError(domain: "ScreenMintCapture", code: 14, userInfo: [NSLocalizedDescriptionKey: microphoneWriter.error?.localizedDescription ?? "Unable to start microphone audio writing"])
			}
		...
		guard assetWriter.startWriting() else {
			throw NSError(domain: "ScreenMintCapture", code: 8, userInfo: [NSLocalizedDescriptionKey: assetWriter.error?.localizedDescription ?? "Unable to start video writing"])
		}
	...
	func stopCapture() async throws -> String {
		guard isRecording else {
			throw NSError(domain: "ScreenMintCapture", code: 9, userInfo: [NSLocalizedDescriptionKey: "No recording in progress"])
		}
...
final class RecorderService {
	...
	private let queue = DispatchQueue(label: "screenmint.screencapturekit.commands")
